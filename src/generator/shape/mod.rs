...
impl<'a> ShapeGenerator<'a> {
    ...
    pub fn generate_random_shape(&mut self, color: String, opacity: f32, target_size: usize) -> Shape {
    ...
    pub fn generate_shapes(&mut self, colors: Vec<String>, opacity: f32, count: usize, size_range: (usize, usize)) -> Vec<Shape> {
    ...
    fn generate_shape_avoiding_cells(&mut self, color: String, opacity: f32, target_size: usize, used_cells: &HashSet<usize>) -> Shape {
        let mut shape = Shape::new(color, opacity);
        let total_cells = self.grid.cell_count();
