...
use crate::generator::{Generator, grid::TriangularGrid};
use crate::Result;
use std::fs;
use std::path::Path;
use svg::Document;
...
fn create_hexagon_boundary(grid: &TriangularGrid) -> Polygon {
    ...
    let points = hex_grid.vertices.iter()
    ...
    for (i, edge1) in edges.iter().enumerate() {
        ...
        for (j, edge2) in edges.iter().enumerate() {
            if i != j {
                ...
                if (edge1.0.x - edge2.1.x).abs() < 1e-6 && (edge1.0.y - edge2.1.y).abs() < 1e-6 &&
                   (edge1.1.x - edge2.0.x).abs() < 1e-6 && (edge1.1.y - edge2.0.y).abs() < 1e-6 {
                    is_boundary = false;
                    break;
                }
            }
